...
(function (isNode) {
    ...
    astgen = {
        ...
        logicalOr: function(left, right) { return { type: SYNTAX.LogicalExpression.name, operator: '||', left: left, right: right }; },
        assignment: function(left, right) { return { type: SYNTAX.AssignmentExpression.name, operator: '=', left: left, right: right }; },
        emptyObject: function() { return { type: SYNTAX.ObjectExpression.name, properties: {} }; }
    };
    ...
    Instrumenter.prototype = {
        ...
        coverStatement: function (node, walker) {
            ...
            if (node.type === SYNTAX.FunctionDeclaration.name) {
                ...
                createStatement = astgen.statement(
                    astgen.assignment(
                        astgen.subscript(
                            astgen.dot(astgen.variable(this.currentState.trackerVar), astgen.variable('s')),
                            astgen.dot(astgen.variable('__coverage__'), astgen.variable('__current_test__'))
                        ),
                        astgen.logicalOr(
                            astgen.subscript(
                                astgen.dot(astgen.variable(this.currentState.trackerVar), astgen.variable('s')),
                ...
                checkStatementExistsStatement = astgen.statement(
                    astgen.assignment(
                        astgen.subscript(
                            astgen.subscript(
                                astgen.dot(astgen.variable(this.currentState.trackerVar), astgen.variable('s')),
                ...
                this.splice(createStatement, node, walker);
                this.splice(checkStatementExistsStatement, node, walker);
